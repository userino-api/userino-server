...
describe('/dashboard:/app/:id/config [POST]', () => {
  let app: TestApp
  ...
  it('works', async () => {
    const { status, body } = await agent.post(`/firebase/app/${app.id}/config`).send({
  ...
  it('invalid config sent', async () => {
    const { status } = await agent.post(`/firebase/app/${app.id}/config`).send({
