...
describe('/dashboard:/project/:id/config [GET]', () => {
  ...
  before(async () => {
    ...
    await firebaseAppConfigModel.create({ project_id: app.project.id, client_id: v4(), client_secret: 'yyy' })
  })

  it('works', async () => {
    const { status, body } = await agent.get(`/media/project/${app.id}/config`)
